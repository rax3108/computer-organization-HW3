...
void cache_sim_t::init()
{
  ...
  for(size_t i=0;i<sets;i++){
    lru_vec[i].resize(ways);
    for(size_t j= 0; j< ways; j++){
      lru_vec[i][j]+= static_cast<size_t>(j);
    }
  }
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      ...
      //move i to last and repair space
      size_t temp = lru_vec[idx][point];
      for(size_t j= point; j< ways - 1; j++){
        lru_vec[idx][j] = lru_vec[idx][j+1];
      }
      lru_vec[idx][ways-1] = temp;
...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  uint64_t victim = tags[idx*ways + lru_vec[idx][0]];
  //up is find lru way(stored in first of vec)
  //down is move toward vec and change newest to last
  size_t temp = lru_vec[idx][0];
  for(size_t j= 0; j< ways - 1; j++){
    lru_vec[idx][j] = lru_vec[idx][j+1];
  }
  lru_vec[idx][ways - 1] = temp;
  
  tags[idx*ways + lru_vec[idx][ways - 1]] = (addr >> idx_shift) | VALID;
  return victim;
}
