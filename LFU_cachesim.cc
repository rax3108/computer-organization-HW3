...
void cache_sim_t::init()
{
  ...
  for(size_t i=0;i<sets;i++){
    lfu_vec[i].resize(ways);
    for(size_t j= 0; j< ways; j++){
      lfu_vec[i][j].str_way = static_cast<size_t>(j);
      lfu_vec[i][j].str_freq = 0;
    }
  }
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      ...
      size_t point;
      for(size_t j= 0; j< ways; j++){
        if(lfu_vec[idx][j].str_way == i){
          lfu_vec[idx][j].str_freq++;
          point = j;
        }
      }
      std::stable_sort(lfu_vec[idx].begin(), lfu_vec[idx].end(), _cond);
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  //fifo
  uint64_t victim = tags[idx*ways + lfu_vec[idx][0].str_way];
  lfu_vec[idx][0].str_freq = 0;
  tags[idx*ways + lfu_vec[idx][0].str_way] = (addr >> idx_shift) | VALID;
  
  //put it to last so that sort can make sure it is last of the fifo order
  struct way_freq temp = lfu_vec[idx][0];
  for(int j= 0; j< ways - 1; j++){
    lfu_vec[idx][j] = lfu_vec[idx][j+1];
  }
  lfu_vec[idx][ways - 1] = temp;
  //sort
  std::stable_sort(lfu_vec[idx].begin(), lfu_vec[idx].end(), _cond);
