...
void cache_sim_t::init()
{
  ...
  self_vec.resize(sets);
  for(size_t i=0;i<sets;i++){
    self_vec[i].resize(ways);
    for(size_t j= 0; j< ways; j++){
      self_vec[i][j] = static_cast<size_t>(j);
    }
  }
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      ...
      size_t temp = self_vec[idx][point];
      self_vec[idx].erase(self_vec[idx].begin() + point);
      self_vec[idx].push_back(0);
      for(size_t j = ways - 1; j > 0; j--)
        self_vec[idx][j] = self_vec[idx][j - 1];
      self_vec[idx][0] = temp;
...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  uint64_t victim = tags[idx*ways + self_vec[idx][0]];
  tags[idx*ways + self_vec[idx][0]] = (addr >> idx_shift) | VALID;
  size_t temp = self_vec[idx][0];
  for(size_t j= 0; j< ways - 1; j++){
    self_vec[idx][j] = self_vec[idx][j+1];
  }
  self_vec[idx][ways - 1] = temp;
  return victim;
}
