...
void cache_sim_t::init()
{
  ...
  //make the vector in size sets
  fifo_vec.resize(sets);
...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  //according saved pointer to select victim block(defualt=0)
  uint64_t victim = tags[idx*ways + fifo_vec[idx]];
  //replace vimctim by addr
  tags[idx*ways + fifo_vec[idx]] = (addr >> idx_shift) | VALID;
  //poiter+1
  fifo_vec[idx] = (fifo_vec[idx] + 1) % ways;
  return victim;
}
